...
def get_file_chunks(file_name: str, max_cpu: int = 8) -> list:
...
def _process_file_chunk(file_name: str, chunk_start: int, chunk_end: int, blocksize: int = 1024 * 1024) -> dict:
        ...
        while byte_count > 0:
            ...
            while data:
                ...
                try:
                    ...
                    result[location] = [value, value, value, 1]  # min, max, sum, count
...
def process_file(cpu_count: int, start_end: list) -> dict:
    ...
    with open("answer.txt", "w") as fi:
        ...
        for location, measurements in sorted(result.items()):
                ...
                f"{(measurements[2] / measurements[3]) if measurements[3] != 0 else 0:.1f}/"
                f"{measurements[1]:.1f}\n "
            )
        fi.write("\b\b} ")
...
if __name__ == "__main__":
    ...
    print(f"Thời gian chạy ifnesi_1brc___: {elapsed_time:.4f} giây")
